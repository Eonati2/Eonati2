...
| 2 | Trigger analysis | Attio → Make → Claude → Attio | Weekly | Evidence required |
...
### 2 — Trigger analysis · Attio → Make → Claude → Attio · weekly

The highest-value job in the list, because trigger hunting is what gets skipped when busy.

Claude reads the account, looks for the trigger types in `revenue-engine` §5, and writes
`Trigger_Event` rows. **Every row stores the verbatim observation, its source, and its date.** A
trigger is an event, not a conclusion — Claude never writes "they need a cleaner", only what was
observed and when.

Runs weekly, not daily: facility events do not appear hourly and the API cost is real.
...
Moves free-kit subscribers into the nurture sequence
(`outbound/sequences/free-product-nurture.md`).

**Sends from the primary business domain, not a cold-sending domain.** A subscriber asked for the
kit; that is warm mail and it belongs on the identity we are building, not on replaceable
...
- Qualified free-kit downloads, by channel, against the requirement in
